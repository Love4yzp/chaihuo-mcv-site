...
5. If the change affects UI, layout, animation, map, carousel, or 3D: run `pnpm visual`.
...
- `pnpm visual` checks desktop and mobile visual substance, horizontal overflow, runtime errors, captures screenshots in the Playwright report, and verifies the 3D page has nonblank WebGL pixels.
...
- Visual failures indicate blank or collapsed pages, overflow, broken WebGL, or runtime errors. Open `playwright-report/` or inspect the attached screenshots.

## Known Limits

The visual harness is a smoke harness, not pixel-perfect screenshot regression. It is designed to help AI agents catch blank renders, broken layout, missing content, mobile overflow, and non-rendering WebGL without making intentional design changes painful.
